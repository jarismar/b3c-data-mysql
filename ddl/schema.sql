...
CREATE TABLE IF NOT EXISTS `b3c`.`broker_invoice_item_sale` (
  ...
  PRIMARY KEY (`biis_id`),
  INDEX `IX_FK_BII_ID_IN` (`bii_id_in` ASC) INVISIBLE,
  INDEX `IX_FK_BII_ID_OUT` (`bii_id_out` ASC) VISIBLE,
  CONSTRAINT `FK_BIIS_BII_IN`
    FOREIGN KEY (`bii_id_in`)
    REFERENCES `b3c`.`broker_invoice_item` (`bii_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_BIIS_BII_OUT`
